...
const InfractionDetail = () => {
  ...
  const fetch = async (nic: string, processNumber: string) => {
    const res = await client.getInfractionDetail(nic, processNumber);
  ...
  const submitHandler = (e: FormEvent) => {
    ...
    const processNumber = data.get("process-number")?.toString();

    if (nic && processNumber) {
      fetch(nic, processNumber);
    }
  };
